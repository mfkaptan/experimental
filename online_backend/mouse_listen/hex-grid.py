...
mouse_listener = server.get_mouse_listener()
...
def display():
    ...
    # Take a screenshot from the server side
    server.set_screenshot(screenshot())
    glut.glutSwapBuffers()

    # If left click
    if mouse_listener.button_isdown is True:
        x, y = mouse_listener.get_coord()
        on_motion(x, y)

    # If mouse's position is updated
    elif mouse_listener.isupdated is True:
        x, y = mouse_listener.get_coord()
        on_passive_motion(x, y)
